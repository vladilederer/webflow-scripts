...
function appendUTMParamsToURL() {
    ...
    // Liste der UTM-Parameter
    let utmKeys = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'hsa_ad', 'hsa_cam', 'hsa_grp', 'campaign_id', 'add_id_1', 'add_id_2', 'account_id'];

    // Debugging: Zeige an, was in der aktuellen URL enthalten ist
    console.log('Aktuelle URL-Parameter:', Array.from(urlParams.entries()));

    // Prüfe, ob einer der UTM-Parameter bereits in der URL vorhanden ist
    let paramsExist = utmKeys.some(key => urlParams.has(key));

    // Debugging: Zeige an, ob UTM-Parameter bereits in der URL existieren
    console.log('Existieren UTM-Parameter in der URL:', paramsExist);

    // Wenn keine UTM-Parameter in der URL sind, füge sie hinzu
    if (!paramsExist) {
        // Hole die UTM-Parameter aus dem sessionStorage
        utmKeys.forEach(key => {
            let storedValue = sessionStorage.getItem(key);
            // Debugging: Zeige an, welche UTM-Parameter im sessionStorage gefunden wurden
            console.log(`UTM-Parameter ${key} im sessionStorage:`, storedValue);

            if (storedValue) {
                urlParams.set(key, storedValue);
            }
        });
        ...
        // Debugging: Zeige die neue URL an, bevor sie angewendet wird
        console.log('Neue URL mit angehängten UTM-Parametern:', newURL);

        // Aktualisiere die URL ohne die Seite neu zu laden
        window.history.replaceState({}, '', newURL);
    } else {
        console.log('UTM-Parameter bereits in der URL vorhanden. Keine Änderung erforderlich.');
    }
}

// Stelle sicher, dass das Skript nach dem Laden der Seite ausgeführt wird
window.addEventListener('load', function() {
    console.log('Page fully loaded. Running appendUTMParamsToURL.');
