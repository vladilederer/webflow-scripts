...
window.addEventListener('load', () => {
    // Get UTM parameters from the local storage of the parent domain
    const utmData = JSON.parse(localStorage.getItem('session'))?.utm;
    
    // Ensure there's data to send
    if (utmData) {
        // Send the UTM data to the iframe (assuming you have only one iframe)
        const iframe = document.querySelector('iframe');
        if (iframe) {
            iframe.contentWindow.postMessage({
                type: 'utmData',
                data: utmData
            }, '*');  // Replace '*' with the iframe's origin for enhanced security
        }
    }
});
