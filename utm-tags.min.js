...
window.addEventListener('load', function () {
    // UTM-Parameter aus der lokalen Session abrufen
    const utmData = JSON.parse(localStorage.getItem('session'))?.utm;

    // Prüfen, ob UTM-Daten vorhanden sind
    if (utmData) {
        // Baue die UTM-Parameter als Query-String
        const queryParams = new URLSearchParams(utmData).toString();

        // Finde das iFrame (verwende eine ID oder einen anderen Selektor, der zu deinem iFrame passt)
        const iframe = document.querySelector('iframe[src^="https://ctq1ykyt.paperform.co"]');

        // Aktualisiere die URL des iFrames mit den UTM-Parametern
        if (iframe) {
            const baseUrl = iframe.src.split('?')[0]; // Entferne alle bestehenden Query-Parameter
            iframe.src = `${baseUrl}?${queryParams}`; // Füge die neuen UTM-Parameter hinzu
        }
    }
});
