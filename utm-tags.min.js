...
function sendUTMToIframe() {
    ...
    if (sessionData) {
        ...
        // Greife auf die UTM-Parameter im "utm"-Objekt innerhalb der Session zu
        if (parsedSession && parsedSession.utm) {
            utmParams = {
                utm_source: parsedSession.utm.utm_source || null,
                utm_medium: parsedSession.utm.utm_medium || null,
                utm_campaign: parsedSession.utm.utm_campaign || null,
                utm_term: parsedSession.utm.utm_term || null,
                utm_content: parsedSession.utm.utm_content || null,
                hsa_ad: parsedSession.utm.hsa_ad || null,
                hsa_cam: parsedSession.utm.hsa_cam || null,
                hsa_grp: parsedSession.utm.hsa_grp || null,
                campaign_id: parsedSession.utm.campaign_id || null,
                add_id_1: parsedSession.utm.add_id_1 || null,
                add_id_2: parsedSession.utm.add_id_2 || null,
                account_id: parsedSession.utm.account_id || null
            };
        }
    } else {
    ...
    // Debugging: Zeige die extrahierten UTM-Parameter
    console.log('UTM-Parameter aus der Session:', utmParams);

    // Selektiere das Paperform-iFrame
    let iframe = document.querySelector('iframe');

    if (iframe) {
        // Übermittle die UTM-Parameter mit postMessage
