...
(() => {
    ...
    function getUTMParamsFromURL() {
        ...
        // Wenn UTM-Parameter vorhanden sind, speichere sie im LocalStorage
        if (Object.keys(utmParams).length > 0) {
            localStorage.setItem(UTM_STORAGE_KEY, JSON.stringify(utmParams));
        }

        // Debug: Zeige die UTM-Parameter im Konsolen-Log an
        console.log('UTM-Parameter aus URL:', utmParams);

        return utmParams;
    }

    // Funktion, um UTM-Parameter aus dem LocalStorage zu holen
    ...
    // Funktion, um automatisch auf eine Seite weiterzuleiten und UTM-Parameter zu übergeben
    function redirectWithUTM(targetUrl) {
        const utmParams = getUTMParamsFromLocalStorage();
        const utmQueryString = new URLSearchParams(utmParams).toString();

        if (utmQueryString) {
            // Debug: Zeige die Weiterleitungs-URL im Konsolen-Log an
            console.log('Weiterleitung an URL:', `${targetUrl}?${utmQueryString}`);

            // Weiterleitung zur neuen Seite mit den UTM-Parametern
            window.location.href = `${targetUrl}?${utmQueryString}`;
        } else {
            console.error('Keine UTM-Parameter gefunden.');
        }
    }

    // Event Listener hinzufügen, um den Button-Link zu aktualisieren, wenn die Seite geladen ist
    document.addEventListener('DOMContentLoaded', function() {
        // UTM-Parameter von der URL holen und speichern
        const utmParams = getUTMParamsFromURL();

        // Aktualisiere den Button-Link
        updateButtonLink();

        // Falls erforderlich, automatische Weiterleitung zu einer bestimmten URL
        const targetUrl = '/geschaftsfuhrer-call'; // Beispielhafte Ziel-URL
        redirectWithUTM(targetUrl); // Diese Zeile kannst du aktivieren, wenn du eine automatische Weiterleitung möchtest
    });
})();
