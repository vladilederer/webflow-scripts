...
<script>
(function() {
  // 1. UTM-Params aus sessionStorage holen
  function getUTM() {
    const session = JSON.parse(localStorage.getItem('session') || '{}');
    return session.utm || {};
  }

  // 2. UTM-Params auf eine URL anwenden
  function appendUTMToURL(rawUrl) {
    try {
      ...
      return rawUrl; // falls ungültige URL
    }
  }

  // 3. Sobald DOM bereit ist, alle Links und Formulare anpassen
  document.addEventListener('DOMContentLoaded', () => {
    const utm = getUTM();
    if (!Object.keys(utm).length) return; // nichts zu tun

    // a) Alle internen Links erweitern
    document.querySelectorAll('a[href]').forEach(a => {
      // nur eigene Domain oder relative Links
      if (a.href.startsWith(window.location.origin) || a.getAttribute('href').startsWith('/')) {
        a.href = appendUTMToURL(a.href);
      }
    });

    // b) Alle Formulare vor dem Absenden anpassen
    document.querySelectorAll('form').forEach(form => {
      form.addEventListener('submit', () => {
        form.action = appendUTMToURL(form.action || window.location.href);
      });
    });
  });
})();
