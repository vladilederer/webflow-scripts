(() => {
    var R = ({ customFormFields: t = {} }) => {
        let r = {
            ...
            rl_campaign_id: t?.campaign_id || "rl_campaign_id",  // New parameter
            rl_add_id_1: t?.add_id_1 || "rl_add_id_1",          // New parameter
            rl_add_id_2: t?.add_id_2 || "rl_add_id_2",          // New parameter
            rl_account_id: t?.account_id || "rl_account_id",    // New parameter
        },
        m = Object.values(r),
        n = {
            ...
            [r.rl_campaign_id]: e.campaign_id,    // New parameter
            [r.rl_add_id_1]: e.add_id_1,          // New parameter
            [r.rl_add_id_2]: e.add_id_2,          // New parameter
            [r.rl_account_id]: e.account_id,      // New parameter
        };
        return {
    ...
    e = {
        ...
        add_id_1: "add_id_1",          // New parameter
        add_id_2: "add_id_2",          // New parameter
        account_id: "account_id",      // New parameter
    },
    f = Object.values(e),
    ...
    l = {
        ...
        [e.hsa_cam]: "",
        [e.hsa_grp]: "",
        [e.campaign_id]: "",     // Default value for the new parameter
        [e.add_id_1]: "",        // Default value for the new parameter
        [e.add_id_2]: "",        // Default value for the new parameter
        [e.account_id]: "",      // Default value for the new parameter
    },
    S = {
    ...
    function E(t) {
        return {
            ...
            [e.campaign_id]: t.get(e.campaign_id),    // New parameter
            [e.add_id_1]: t.get(e.add_id_1),          // New parameter
            [e.add_id_2]: t.get(e.add_id_2),          // New parameter
            [e.account_id]: t.get(e.account_id),      // New parameter
        };
    }
    function D(t) {
        let r = E(t);
    ...
    function x(t) {
        return {
            ...
            [e.campaign_id]: t.get(e.campaign_id) || l[e.campaign_id],    // New parameter
            [e.add_id_1]: t.get(e.add_id_1) || l[e.add_id_1],              // New parameter
            [e.add_id_2]: t.get(e.add_id_2) || l[e.add_id_2],              // New parameter
            [e.account_id]: t.get(e.account_id) || l[e.account_id],        // New parameter
        };
    }
    function O() {
    ...
    function w({ utmParams: t, sessionExpirationTime: r }) {
        ...
        // PostMessage hinzugefügt, um UTM-Parameter an das Paperform-iFrame zu senden
...
(() => {
    // Funktion, um UTM-Parameter aus der URL zu holen
    function getUTMParams() {
        const urlParams = new URLSearchParams(window.location.search);
        const utmParams = {};

        // Prüfe alle relevanten UTM-Parameter
        ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'hsa_ad', 'hsa_cam', 'hsa_grp', 'campaign_id', 'add_id_1', 'add_id_2', 'account_id'].forEach(param => {
            if (urlParams.has(param)) {
                utmParams[param] = urlParams.get(param);
            }
        });

        // Debug: Zeige die UTM-Parameter im Konsolen-Log an
        console.log('UTM-Parameter:', utmParams);

        return new URLSearchParams(utmParams).toString();
    }

    // Funktion, um den Button-Link mit UTM-Parametern zu aktualisieren
    function updateButtonLink() {
        const utmQueryString = getUTMParams();

        // Debug: Zeige die generierte Query-String im Konsolen-Log an
        console.log('UTM Query-String:', utmQueryString);

        if (utmQueryString) {
            // Finde das a-Tag (den Button) basierend auf der Klasse und aktualisiere den href
            const button = document.querySelector('.button-copy.w-button');
            
            // Debug: Überprüfe, ob der Button gefunden wurde
            if (button) {
                console.log('Button gefunden:', button);
                const baseUrl = button.getAttribute('href').split('?')[0]; // Entferne bestehende Query-Parameter
                button.setAttribute('href', `${baseUrl}?${utmQueryString}`); // Füge die neuen UTM-Parameter hinzu

                // Debug: Zeige den aktualisierten href-Wert des Buttons an
                console.log('Neuer Button href:', button.getAttribute('href'));
            } else {
                console.error('Button nicht gefunden. Überprüfe den Selektor.');
            }
        }
    }

    // Event Listener hinzufügen, um den Button-Link zu aktualisieren, wenn die Seite geladen ist
