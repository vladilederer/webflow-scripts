...
(() => {
    const UTM_STORAGE_KEY = 'session';

    // Funktion, um UTM-Parameter aus dem LocalStorage zu holen
    function getUTMParamsFromLocalStorage() {
        const storedSession = localStorage.getItem(UTM_STORAGE_KEY);
        
        // Prüfe, ob die UTM-Parameter im "session"-Key vorhanden sind und korrekt geparst werden können
        if (storedSession) {
            try {
                const parsedSession = JSON.parse(storedSession);
                if (parsedSession && parsedSession.utm) {
                    console.log('UTM-Parameter aus LocalStorage:', parsedSession.utm);
                    return parsedSession.utm;
                }
            } catch (error) {
                console.error('Fehler beim Parsen der UTM-Parameter:', error);
            }
        }
        return {};
    }

    // Funktion, um UTM-Parameter an eine URL anzuhängen
    function appendUTMParamsToURL(baseUrl) {
        const utmParams = getUTMParamsFromLocalStorage();
        const utmQueryString = new URLSearchParams(utmParams).toString();

        if (utmQueryString) {
            const newUrl = `${baseUrl}?${utmQueryString}`;
            console.log('Neue URL mit UTM-Parametern:', newUrl);
            return newUrl;
        }
    ...
    function redirectWithUTM(targetUrl) {
        const newUrl = appendUTMParamsToURL(targetUrl);

        // Debug: Zeige die Weiterleitungs-URL im Konsolen-Log an
        console.log('Weiterleitung an URL:', newUrl);

        // Weiterleitung zur neuen Seite mit den UTM-Parametern
        window.location.href = newUrl;
    }

    // Event Listener hinzufügen, um die Weiterleitung zu prüfen, wenn die Seite geladen ist
    document.addEventListener('DOMContentLoaded', function() {
        // Beispielhafte Ziel-URL: die URL, auf die du die Benutzer weiterleiten möchtest
        const targetUrl = 'https://www.sales-elements.io/landing/vsl-whitepaper'; 

        // Weiterleitung mit UTM-Parametern
