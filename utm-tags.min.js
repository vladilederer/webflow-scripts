...
<script>
(function() {
  // 1) UTM-Params aus der Anfangs-URL holen
  const initialParams = new URLSearchParams(window.location.search);
  const utmKeys = ['utm_source','utm_medium','utm_campaign','utm_term','utm_content','hsa_ad','hsa_cam','hsa_grp','campaign_id','add_id_1','add_id_2','account_id','fbc_id','h_ad_id','lnk_id'];
  const utm = {};
  utmKeys.forEach(k => {
    const v = initialParams.get(k);
    if (v) utm[k] = v;
  });
  if (!Object.keys(utm).length) return; // keine UTMs in URL

  // 2) Hilfsfunktion: URL um UTMs erweitern
  ...
  // 3) Alle <a> und Buttons mit href updaten
  window.addEventListener('load', () => {
    // a) alle Links
    document.querySelectorAll('a[href]').forEach(a => {
      // nur interne oder relative Links
      if (a.href.startsWith(window.location.origin) || a.getAttribute('href').startsWith('/')) {
        a.href = withUTM(a.href);
      }
    });
    // b) alle Buttons, die ein href haben
    document.querySelectorAll('button').forEach(btn => {
      if (btn.getAttribute('href')) {
        btn.setAttribute('href', withUTM(btn.getAttribute('href')));
      }
    });
  });
})();
