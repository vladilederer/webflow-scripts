(() => {
    var R = ({ customFormFields: t = {} }) => {
        let r = {
            ...
            rl_fbc_id: t?.fbc_id || "rl_fbc_id",
            rl_h_ad_id: t?.h_ad_id || "rl_h_ad_id",
            rl_lnk_id: t?.lnk_id || "rl_lnk_id",  // LinkedIn Hyros UTM-ID hinzugefügt
        },
        m = Object.values(r),
        n = {
            ...
            [r.rl_fbc_id]: e.fbc_id,
            [r.rl_h_ad_id]: e.h_ad_id,
            [r.rl_lnk_id]: e.lnk_id,  // LinkedIn Hyros UTM-ID hinzugefügt
        };
        return {
    ...
    e = {
        ...
        fbc_id: "fbc_id",
        h_ad_id: "h_ad_id",
        lnk_id: "lnk_id", // LinkedIn Hyros UTM-ID hinzugefügt
    },
    f = Object.values(e),
    ...
    l = {
        ...
        [e.fbc_id]: "",
        [e.h_ad_id]: "",
        [e.lnk_id]: "", // LinkedIn Hyros UTM-ID hinzugefügt
    },
    S = {
    ...
    function E(t) {
        return {
            ...
            [e.fbc_id]: t.get(e.fbc_id),
            [e.h_ad_id]: t.get(e.h_ad_id),
            [e.lnk_id]: t.get(e.lnk_id), // LinkedIn Hyros UTM-ID hinzugefügt
        };
    }
    function D(t) {
        let r = E(t);
    ...
    function x(t) {
        return {
            ...
            [e.fbc_id]: t.get(e.fbc_id) || l[e.fbc_id],
            [e.h_ad_id]: t.get(e.h_ad_id) || l[e.h_ad_id],
            [e.lnk_id]: t.get(e.lnk_id) || l[e.lnk_id], // LinkedIn Hyros UTM-ID hinzugefügt
        };
    }
    function O() {
    ...
    function N({ referrer: t = "", sessionExpirationTime: r }) {
        ...
        let u = {
            ...
            [e.campaign_id]: S[e.campaign_id],
            [e.add_id_1]: S[e.add_id_1],
            [e.add_id_2]: S[e.add_id_2],
            [e.account_id]: S[e.account_id],
            [e.fbc_id]: S[e.fbc_id],
            [e.h_ad_id]: S[e.h_ad_id],
            [e.lnk_id]: S[e.lnk_id], // LinkedIn Hyros UTM-ID hinzugefügt
        };
...
function appendUTMToButtonLinks() {
    ...
    // Liste der spezifischen Selektoren
    let buttonClasses = [
    ...
    // Verzögerung, um sicherzustellen, dass alle Buttons auf der Seite geladen sind
    setTimeout(function () {
        // Gehe durch alle Button-Klassen und hänge die UTM-Parameter an die URL
        buttonClasses.forEach(buttonClass => {
            let buttons = document.querySelectorAll(buttonClass);

            // Debugging: Protokolliere die gefundenen Buttons
            console.log(`Buttons mit dem Selektor ${buttonClass}:`, buttons);
            ...
            if (buttons.length > 0) {
                buttons.forEach(button => {
                    button.addEventListener('click', function(event) {
                        ...
                        // UTM-Parameter zur URL hinzufügen
                        ...
                        // Setze die aktualisierte URL im Button-Href und leite weiter
                        button.href = finalURL;
                        window.location.href = finalURL;
                    });
                });
            } else {
                console.log(`Keine Buttons mit dem Selektor ${buttonClass} gefunden.`);
            }
        });
    }, 500); // Füge eine Verzögerung von 500ms hinzu
}
...
<script>
(function(){
  // 1) Liste aller UTM-Keys, die wir speichern & weitergeben wollen
  const utmKeys = [
    'utm_source','utm_medium','utm_campaign','utm_term','utm_content',
    'hsa_ad','hsa_cam','hsa_grp','campaign_id','add_id_1','add_id_2',
    'account_id','fbc_id','h_ad_id','lnk_id'
  ];

  // 2) Hilfsfunktionen für Cookie-Zugriff
  function setCookie(name, value, days=1) {
    const expires = new Date(Date.now() + days*864e5).toUTCString();
    document.cookie = name + '=' + encodeURIComponent(value) +
                     '; domain=.sales-elements.io; path=/; expires=' + expires;
  }
  ...
  let hasUTM = utmKeys.some(k => urlParams.has(k));

  // 4) Wenn URL UTM enthält → in Cookie speichern
  if (hasUTM) {
    setCookie('utm', urlParams.toString());
  } else {
    // 5) Wenn keine UTM in URL, aber im Cookie → URL ohne Reload updaten
    const cookie = getCookie('utm');
    if (cookie) {
      const cp = new URLSearchParams(cookie);
      const newUrl = new URL(window.location);
      cp.forEach((v,k) => newUrl.searchParams.set(k,v));
      window.history.replaceState({},'', newUrl);
      // nun sind die UTM in der URL
    }
  }

  // 6) Nach DOM-Load alle internen Links updaten
  document.addEventListener('DOMContentLoaded', () => {
    const finalParams = new URLSearchParams(window.location.search);
    // nichts tun, wenn weiterhin keine UTM
    if (!utmKeys.some(k => finalParams.has(k))) return;

    document.querySelectorAll('a[href]').forEach(a => {
      try {
        const u = new URL(a.href, window.location);
        // nur eigene Domain/Subdomains
        if (u.hostname.endsWith('sales-elements.io')) {
          utmKeys.forEach(k => {
            const v = finalParams.get(k);
