...
function populateUTMFields() {
    // Hole die session-Daten aus dem sessionStorage
   ...
   function populateUTMFields() {
    ...
    // Wähle das iFrame (Paperform) aus
    let iframe = document.querySelector('iframe');
    if (iframe) {
        let iframeDocument = iframe.contentWindow.document;

        // Fülle die Hidden Fields auf Seite 2 im iFrame mit den UTM-Daten
        iframeDocument.querySelector('input[name="utm_source"]').value = utmParams.utm_source;
        iframeDocument.querySelector('input[name="utm_campaign"]').value = utmParams.utm_campaign;
        iframeDocument.querySelector('input[name="utm_ad"]').value = utmParams.utm_ad;
        iframeDocument.querySelector('input[name="utm_term"]').value = utmParams.utm_term;
        iframeDocument.querySelector('input[name="utm_content"]').value = utmParams.utm_content;
        iframeDocument.querySelector('input[name="utm_medium"]').value = utmParams.utm_medium;
        iframeDocument.querySelector('input[name="hsa_ad"]').value = utmParams.hsa_ad;
        iframeDocument.querySelector('input[name="hsa_cam"]').value = utmParams.hsa_cam;
        iframeDocument.querySelector('input[name="hsa_grp"]').value = utmParams.hsa_grp;

        console.log('UTM-Daten wurden an die Hidden Fields auf Seite 2 übergeben:', utmParams);
    } else {
        console.log('Paperform-iFrame nicht gefunden.');
    }
}

// Event-Listener, um den Seitenwechsel im Paperform-Formular zu erkennen
function watchForPageChange() {
    let iframe = document.querySelector('iframe');
    if (iframe) {
        let iframeWindow = iframe.contentWindow;

        // Überprüfe alle 500ms, ob Seite 2 angezeigt wird
        let checkInterval = setInterval(function() {
            let currentPage = iframeWindow.document.querySelector('.current-page');
            
            // Wenn Seite 2 angezeigt wird, populateUTMFields aufrufen
            if (currentPage && currentPage.textContent.includes('Page 2')) {
                populateUTMFields();
                clearInterval(checkInterval);  // Stoppe das Intervall, nachdem Seite 2 erreicht wurde
            }
...
// Starte den Listener nach dem Laden der Seite
window.addEventListener('load', function() {
    console.log('Page fully loaded. Watching for page change to Page 2.');
    watchForPageChange();
});
